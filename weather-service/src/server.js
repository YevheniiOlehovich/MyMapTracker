...
async function decodeAVL(buffer, imei, db) {
  try {
    ...
    const crcPacket = buffer.readUInt16BE(
      buffer.length - 2
    );
    ...
    const timestamp =
      Number(avlBuf.readBigUInt64BE(2)) / 1000;
    ...
    const longitude =
      avlBuf.readInt32BE(gpsOffset) / 1e7;

    const latitude =
      avlBuf.readInt32BE(gpsOffset + 4) / 1e7;

    const altitude =
      avlBuf.readInt16BE(gpsOffset + 8);

    const angle =
      avlBuf.readInt16BE(gpsOffset + 10);

    const satellites =
      avlBuf[gpsOffset + 12];

    const speed =
      avlBuf.readInt16BE(gpsOffset + 13);

    const { io, eventId } = parseCodec8IO(
      avlBuf,
      gpsOffset + 15
    );

    const card_id =
      io[157] && !/^0+$/.test(io[157])
        ? io[157]
        : null;
