...
// Відправка 4-байтового ACK
function sendAck(socket, recCount) {
  const ackBuf = Buffer.alloc(4);
  ackBuf.writeUInt32BE(recCount, 0);
  socket.write(ackBuf);
}
  ...
  try {
    ...
    const server = net.createServer(sock => {
      ...
      sock.on('data', async data => {
        ...
        // Перший пакет — IMEI
        if (!imei) {
          imei = cleanImei(buffer.toString());
          logToFile(`📡 Detected IMEI: ${imei}`);
          buffer = Buffer.alloc(0);
          return;
        }

        while (buffer.length >= 12) { // мінімальний пакет 12 байт (4+4+4)
          const recCount = await decodeAvlData(buffer, imei, db);
          if (recCount === 0) break;
          sendAck(sock, recCount);
          buffer = Buffer.alloc(0); // очищаємо буфер після обробки
        }
      });
