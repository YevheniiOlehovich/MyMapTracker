...
async function start() {
  try {
    ...
    const server = net.createServer(sock => {
      ...
      sock.on('data', async data => {
        if (!imei) {
          logToFile(`📥 FIRST PACKET (IMEI): ${data.toString('hex')}`);
          imei = cleanImei(data.toString());
          logToFile(`📡 IMEI = ${imei}`);
          // Можна надіслати одиничку для підтвердження IMEI
          sock.write(Buffer.from([0x01]));
          logToFile(`➡️ Sent confirmation byte: 01`);
          return;
        }

        logToFile(`📥 AVL DATA: ${data.toString('hex')}`);
        const crcCalc = await decodeAvlData(data, imei, db);

        if (crcCalc !== null) {
          const crcBuffer = Buffer.alloc(2);
          crcBuffer.writeUInt16BE(crcCalc, 0);
          sock.write(crcBuffer);
          logToFile(`➡️ Sent CRC16: ${crcCalc.toString(16).toLowerCase()}`);
        }
      });
