...
import { fetchFields } from '../../store/fieldsSlice'; // Імпорт дії для фетчінгу всіх полів
import apiRoutes from '../../helpres/ApiRoutes'; // Імпорт маршрутів API
import * as turf from '@turf/turf'; // Імпорт бібліотеки turf.js

export default function AddFieldsModal({ field }) {
    console.log('Дані з бази даних:', field); // Виводимо дані, які приходять з бази даних
    const dispatch = useDispatch();
    const [fieldName, setFieldName] = useState('');
    const [mapKey, setMapKey] = useState('');
    const [fieldArea, setFieldArea] = useState('');
    ...
    useEffect(() => {
        if (field) {
            ...
            setBranch(field.properties.branch);
            setRegion(field.properties.region);
            calculateFieldArea(field.geometry.coordinates); // Викликаємо функцію для розрахунку площі поля
        } else {
            setFieldName('');
            setMapKey('');
            setFieldArea('');
    ...
    const calculateFieldArea = (coordinates) => {
        ...
        const calculatedArea = (area / 10000).toFixed(5); // Перетворюємо площу з квадратних метрів на гектари і округлюємо до п'яти знаків після коми
        // setFieldArea(calculatedArea);
        console.log('Розрахована площа поля (га):', calculatedArea); // Виводимо розраховану площу поля у консоль
    };
